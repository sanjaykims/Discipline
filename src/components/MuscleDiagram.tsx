...
export type MuscleStatus = "none" | "week" | "today";

const NEUTRAL_FILL = "#1f2937";
const NEUTRAL_STROKE = "#374151";
const ACCENT = "#10b981";

function fillFor(status: MuscleStatus) {
  if (status === "today") return ACCENT;
  if (status === "week") return ACCENT;
  return NEUTRAL_FILL;
}
function opacityFor(status: MuscleStatus) {
  if (status === "today") return 1;
  if (status === "week") return 0.35;
  return 1;
}

interface Props {
  status: Record<MuscleKey, MuscleStatus>;
}

function Region({
  status, shapes,
}: { status: MuscleStatus; shapes: React.ReactNode }) {
  return (
    <g fill={fillFor(status)} fillOpacity={opacityFor(status)} stroke={NEUTRAL_STROKE} strokeWidth={1.5}>
      {shapes}
    </g>
  );
}

export function MuscleDiagram({ status }: Props) {
  return (
    <div className="flex gap-4 justify-center">
      <FrontBody status={status} />
      <BackBody status={status} />
    </div>
  );
}

function FrontBody({ status }: Props) {
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 200 400" className="w-full max-w-[160px]">
        {/* Head, neck, torso outline, shins, feet — neutral background */}
        <g fill={NEUTRAL_FILL} stroke={NEUTRAL_STROKE} strokeWidth={1.5}>
...
function BackBody({ status }: Props) {
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 200 400" className="w-full max-w-[160px]">
        <g fill={NEUTRAL_FILL} stroke={NEUTRAL_STROKE} strokeWidth={1.5}>
        ...
        <Region status={status.back} shapes={<path d="M62,64 L138,64 L146,112 L136,178 L64,178 L54,112 Z" />} />
        ...
        {/* Triceps */}
        <Region status={status.triceps} shapes={<>
          <rect x="29" y="88" width="22" height="62" rx="11" />
          <rect x="149" y="88" width="22" height="62" rx="11" />
        </>} />
        ...
        {/* Glutes */}
        <Region status={status.glutes} shapes={<rect x="66" y="180" width="68" height="52" rx="18" />} />

        {/* Hamstrings */}
        <Region status={status.hamstrings} shapes={<>
          <rect x="64" y="234" width="34" height="94" rx="15" />
          <rect x="102" y="234" width="34" height="94" rx="15" />
        </>} />

        {/* Calves */}
        <Region status={status.calves} shapes={<>
          <rect x="68" y="330" width="26" height="58" rx="12" />
          <rect x="106" y="330" width="26" height="58" rx="12" />
        </>} />
      </svg>
